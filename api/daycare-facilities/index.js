...
export default async function handler(req, res) {
  ...
  try {
    if (req.method === 'GET') {
      const { rows } = await sql`SELECT * FROM daycare_facilities ORDER BY sort_order ASC, id ASC`;
    ...
    if (req.method === 'POST') {
      ...
      const { rows } = await sql`
        INSERT INTO daycare_facilities (title, description, sort_order)
    ...
    console.error('Daycare facilities error:', err);
